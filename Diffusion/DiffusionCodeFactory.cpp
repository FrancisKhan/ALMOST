...
std::shared_ptr<BaseDiffusionCode> DiffusionCodeFactory::setDiffusionCode(Reactor &reactor, Library &library)
{
  if (reactor.getMesh().getGeometry() == GeomKind::CYLINDER)
  {
    return std::make_shared<CylDiffusionCode>(reactor, library);
  }
  else if (reactor.getMesh().getGeometry() == GeomKind::SPHERE)
  {
    return std::make_shared<SphDiffusionCode>(reactor, library);
  }
  else if (reactor.getMesh().getGeometry() == GeomKind::SLAB)
  {
    return std::make_shared<SlabDiffusionCode>(reactor, library);
  }
  else
  {
